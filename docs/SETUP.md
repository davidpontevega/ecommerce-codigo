...
| Tablas | TanStack Table | 9.x (instalado por bootstrap; API difiere de 8.x) |
| HTTP | Axios | 1.x |
| Estado cliente | Zustand | 5.x |
| Gráficos | Recharts | 3.x (instalado por bootstrap; API difiere de 2.x) |
| Validación | Zod | 4.x |
...
```json
{
  "scripts": {
    ...
    "test": "node --env-file=.env.local --import tsx --test \"src/**/*.test.ts\""
  }
}
```

> `test` no instala nada nuevo: `node:test` + `node:assert` son de Node; `tsx`
> (ya en `devDependencies` por `db:seed`) resuelve TypeScript y los alias `@/*`
> del `tsconfig`. `--env-file=.env.local` (nativo desde Node 20) carga
> `DATABASE_URL`: algunos repositorios importan `src/server/db/index.ts`, que
> lanza si la variable falta al construir el pool de Neon — aunque el test en
> sí no ejecute ninguna query. Ver la convención de ubicación en la sección 3.
...
> **Desviación registrada:** en Next.js 16 el archivo de middleware se llama
> `src/proxy.ts`, no `src/middleware.ts`. El contenido y las reglas de la sección 6
> (protección en el borde) no cambian; solo el nombre del archivo. Donde este
> documento diga `middleware.ts`, léase `proxy.ts`.
...
| Prueba unitaria | `<archivo>.test.ts` | `product.repository.test.ts` |

### Pruebas unitarias

**Co-localizadas: `<archivo>.test.ts` junto al archivo que prueba.** Nada de un
árbol `tests/` ni `__tests__/` aparte que espeje `src/` — un archivo de más por
función a probar, no una carpeta paralela que mantener sincronizada.
...
│       └── <dominio>.repository.test.ts
└── modules/cart/store/
    ├── cart-store.ts
    └── cart-store.test.ts
```

Alcance — **solo funciones puras** (mismo input → mismo output, sin Postgres,
sin Stripe, sin Clerk, sin DOM): ver el inventario ya hecho en
`docs/testing/candidatas-pruebas-unitarias.md`. Nada de UI (`.tsx`), hooks
(`use-*.ts`), Route Handlers ni `server/services/*.ts` — esos son de integración,
no unitarios; no llevan `*.test.ts` bajo esta convención.

Runner: `node:test` + `node:assert` (nativos de Node, cero dependencia nueva),
TypeScript vía `tsx` (ya instalado) — comando en la sección 2 (`npm run test`).

Una función privada que valga la pena probar (p. ej. `buildFilters`,
`comparePriceIsHigher`) se exporta para el test o se prueba indirectamente a
través de la función pública / el schema que la usa — nunca se duplica.

Si en el futuro hacen falta fixtures o helpers compartidos entre varios
`*.test.ts`, van en `src/test/` (p. ej. `src/test/fixtures.ts`). No crear esa
carpeta antes de que haya un segundo archivo que la necesite.
